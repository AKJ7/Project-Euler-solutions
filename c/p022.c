...
static int predicate(const void* a, const void* b)
{
    return strcmp(*(const char**)a, *(const char**)b);
}

static uint32_t alphaValue(const char* ptr)
{
    uint32_t retVal = 0;
    while (*ptr)
        retVal += ascii2alph(*ptr++) + 1;
    return retVal;
}
...
int main(int argc, char** argv) 
{
    ...
    qsort(names, count, sizeof(*names), predicate);
    uint32_t total = 0; 
    for (size_t counter = 0; counter < count; ++counter)
        total += alphaValue(names[counter]) * (counter + 1);
    printf("%"PRIu32, total);
